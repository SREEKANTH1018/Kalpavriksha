...
void main() {
  ...
  float sc =
    mix(1.0, uHoverScale, uHoverAmount)
    + uBurst * 0.3;
  ...
  for (int i = 0; i < 10; i++) {
    ...
    vec2 pr =
      p - fi * uParallax * uMouse;

    vec3 rc =
      mix(uColor, uColorTwo, fi / rcf);
  ...
  gl_FragColor =
    vec4(
      outputColor,
      clamp(outputAlpha * uOpacity, 0.0, 1.0)
    );
}
`;
...
export default function MagicRings({
  ...
  const propsRef =
    useRef<Required<MagicRingsProps> | null>(null);
